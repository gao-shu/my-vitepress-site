...
如果你现在只是普通开发，希望**先快速入门，再一步步深入**，建议走这条路径：

1. **先跑通 7 天快速入门**  
   - [AI 快速入门（7 天）](/ai/quickstart)
2. **再看全局路线图（知道后面学什么）**  
   - [AI 时代开发者入门路线（LLM / Agent / 多模态 / MLOps）](/ai/ai-roadmap)
3. **进入主路径（建议顺序）**  
   - [LLM 入门与 Prompt 规划](/ai/llm-basics) → [Agent / 工具型 AI](/ai/agent-basics) → [MVP Server / 数据中台](/ai/mvp-server)
4. **按需选修进阶**  
   - [多模态 & 知识图谱](/ai/multimodal-kg) / [部署 & MLOps](/ai/mlops-platform)
...
- **第 0 步（3-7 天）**：先完成 [AI 快速入门（7 天）](/ai/quickstart)
- **第 1 步（1-2 周）**：走主路径  
  - [LLM 入门](/ai/llm-basics) → [Agent 入门](/ai/agent-basics)
- **第 2 步（1-3 周）**：加数据中台  
  - [MVP Server](/ai/mvp-server)
- **第 3 步（选修）**：做更聪明/更稳定  
  - [多模态 & 知识图谱](/ai/multimodal-kg) / [部署 & MLOps](/ai/mlops-platform)
