# 开发工具与环境配置

面向 Java 全栈与日常研发：**JDK、IDEA、Maven、Git、Docker、数据库客户端、API 测试**等工具的**安装、配置与常见排查**。内容按主题拆成多篇文章，便于查阅与增量更新。

> 本站另有 [技术体系 · Docker 专题](/tech-system/devops/docker) 偏「知识体系与进阶路线」；本模块的 [Docker 快速上手](./docker) 偏「本机装环境与常用命令」。

## 文档目录

### 开发环境
- [JDK 安装与多版本管理](./jdk)
- [Node.js 安装与版本控制](./nodejs-install-version)
- [Python 安装与版本控制](./python-install-version)
- [本地 MySQL 安装配置](./mysql-local)
- [Redis 安装配置](./redis-local)
- [Docker 快速上手](./docker)

### 开发工具
- [IntelliJ IDEA 配置](./intellij-idea)（含 Maven 联动、插件与 Live Template 备忘）
- [VS Code 安装与配置](./vscode)
- [Cursor 安装与配置](./cursor)（含 MCP 集成用法）
- [GitHub Codespaces](./github-codespaces)（云端开发环境）
- [Figma](./figma)（协作式 UI 设计工具）
- [Maven 配置与最佳实践](./maven)
- [Git 版本控制实战](./git)
- [数据库客户端工具](./database-tools)
- [API 测试工具](./api-testing)
- [其他实用工具](./misc-tools)

### 汇总与扩展
- [一键配置脚本](./setup-scripts)
- [环境常见问题排查](./troubleshooting)
- [工具选型建议](./tool-selection)

---
